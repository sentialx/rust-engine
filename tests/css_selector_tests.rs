...
use graviton::layout::{element_matches_selector, element_matches_selector_with_siblings, Rect, SiblingContext};

// Thread-local frame for creating test elements
thread_local! {
    static TEST_FRAME: Rc<RefCell<Frame>> = Frame::new(Rect { x: 0.0, y: 0.0, width: 800.0, height: 600.0 });
}
...
fn create_element(tag: &str) -> Rc<RefCell<DomElement>> {
    TEST_FRAME.with(|frame| {
        frame.borrow().create_element(tag)
    })
}

/// Helper to create an element with class
fn create_element_with_class(tag: &str, class: &str) -> Rc<RefCell<DomElement>> {
    let el = create_element(tag);
    el.borrow_mut().set_attribute("class", class);
    el
}

/// Helper to create an element with id
fn create_element_with_id(tag: &str, id: &str) -> Rc<RefCell<DomElement>> {
    let el = create_element(tag);
    el.borrow_mut().set_attribute("id", id);
    el
}
...
fn test_tag_selector_matches() {
    ...
    assert!(element_matches_selector(&el.borrow(), &selector, &[]));
}
...
fn test_attribute_exists() {
    ...
    el.borrow_mut().set_attribute("disabled", "true");
...
fn test_attribute_equals() {
    let mut el = create_element("input");
    el.borrow_mut().set_attribute("type", "text");
...
fn test_attribute_equals_no_match() {
    let mut el = create_element("input");
    el.borrow_mut().set_attribute("type", "password");
...
fn test_attribute_contains_word() {
    ...
    el.borrow_mut().set_attribute("data-tags", "foo bar baz");
...
fn test_attribute_starts_with() {
    ...
    el.borrow_mut().set_attribute("href", "https://example.com");
...
fn test_attribute_ends_with() {
    let mut el = create_element("a");
    el.borrow_mut().set_attribute("href", "document.pdf");
...
fn test_attribute_contains() {
    let mut el = create_element("a");
    el.borrow_mut().set_attribute("href", "https://example.com/path");
...
fn test_attribute_dash_match() {
    let mut el = create_element("div");
    el.borrow_mut().set_attribute("lang", "en-US");
...
fn test_attribute_dash_match_exact() {
    let mut el = create_element("div");
    el.borrow_mut().set_attribute("lang", "en");
...
fn test_and_group_tag_class_id() {
    ...
    el.borrow_mut().set_attribute("id", "main");
...
fn test_descendant_combinator_direct_parent() {
    ...
    assert!(element_matches_selector(&child.borrow(), &selector, &[parent_ptr]));
}
...
fn test_descendant_combinator_grandparent() {
    ...
    let grandparent_ptr = (*grandparent).as_ptr();
    let parent_ptr = (*parent).as_ptr();
    assert!(element_matches_selector(&child.borrow(), &selector, &[grandparent_ptr, parent_ptr]));
}
...
fn test_descendant_three_levels() {
    ...
    let a_ptr = (*a).as_ptr();
    let b_ptr = (*b).as_ptr();
    assert!(element_matches_selector(&c.borrow(), &selector, &[a_ptr, b_ptr]));
}
...
fn test_parsed_complex_selector() {
    ...
    let nav_ptr = (*nav).as_ptr();
    assert!(element_matches_selector(&item.borrow(), &selector, &[container_ptr, nav_ptr]));
}
...
fn test_deeply_nested_descendant_selector_manual() {
    ...
    let container_ptr = (*container).as_ptr();
    let main_ptr = (*main).as_ptr();
    let article_ptr = (*article).as_ptr();
    let section_ptr = (*section).as_ptr();

    let parents = vec![body_ptr, container_ptr, main_ptr, article_ptr, section_ptr];
    assert!(element_matches_selector(&h2.borrow(), &selector, &parents));
}
...
fn test_multiple_class_descendant_manual() {
    ...
    assert!(element_matches_selector(&h2.borrow(), &selector, &[body_ptr, content_ptr]));
}
...
fn create_sibling_context(elements: &[Rc<RefCell<DomElement>>], current_index: usize) -> SiblingContext {
...
fn test_empty_no_match() {
    ...
    el.borrow_mut().children.push(child);
...
fn test_nth_child_number() {
    ...
    assert!(!element_matches_selector_with_siblings(&elements[0].borrow(), &selector, &[], Some(&ctx0)));

    // Second child (index 1) should match
    let ctx1 = SiblingContext { index: 1, total: 3, siblings: siblings.clone() };
    assert!(element_matches_selector_with_siblings(&elements[1].borrow(), &selector, &[], Some(&ctx1)));

    // Third child (index 2) should not match
    let ctx2 = SiblingContext { index: 2, total: 3, siblings: siblings.clone() };
    assert!(!element_matches_selector_with_siblings(&elements[2].borrow(), &selector, &[], Some(&ctx2)));
}
...
fn test_nth_child_odd() {
    ...
    assert!(!element_matches_selector_with_siblings(&elements[3].borrow(), &selector, &[], Some(&ctx3)));
}
...
fn test_nth_child_3n() {
    ...
    let elements: Vec<Rc<RefCell<DomElement>>> = (0..6).map(|_| create_element("p")).collect();
    ...
    for i in 0..6 {
        ...
        assert_eq!(
            element_matches_selector_with_siblings(&elements[i].borrow(), &selector, &[], Some(&ctx)),
            expected,
...
fn test_not_class() {
    ...
    assert!(!element_matches_selector(&el_with_class.borrow(), &selector, &[]));
    assert!(element_matches_selector(&el_without_class.borrow(), &selector, &[]));
}
...
fn test_not_tag() {
    ...
    assert!(element_matches_selector(&div.borrow(), &selector, &[]));
    assert!(!element_matches_selector(&span.borrow(), &selector, &[]));
}
...
fn test_not_id() {
    ...
    assert!(!element_matches_selector(&el_with_id.borrow(), &selector, &[]));
    assert!(element_matches_selector(&el_without_id.borrow(), &selector, &[]));
}
